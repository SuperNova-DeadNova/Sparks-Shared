...
{    
    public sealed class GlobalDiscordSession { public string ID, LastSeq; }
    ...
    public sealed class GlobalDiscordWebsocket : ClientWebSocket 
    {       
        /// <summary> Authorisation token for the bot account </summary>
        public string Token;
        public bool CanReconnect = true;
        public GlobalDiscordSession Session;
        ...
        public override void OnDisconnected(int reason) {
            if (reason == REASON_INVALID_TOKEN) {
                Logger.Log(LogType.Warning, "GlobalDiscord relay: Invalid bot token provided - unable to connect");
                CanReconnect = false;
            }
            
            Logger.Log(LogType.SystemActivity, "GlobalDiscord relay bot closing: " + reason);
            Close();
        }
        ...
        void DispatchPacket(int opcode, JsonObject obj) {
            if (opcode == OPCODE_DISPATCH) {
                ...
                Logger.Log(LogType.Warning, "GlobalDiscord relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
