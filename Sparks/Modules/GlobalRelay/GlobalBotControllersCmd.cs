...
namespace GoldenSparks.Modules.GlobalRelay 
{
    public abstract class GlobalBotControllersCmd : Command2 
    {
        public override string type { get { return CommandTypes.Moderation; } }
        public override LevelPermission defaultRank { get { return LevelPermission.Admin; } }
        public abstract GlobalRelayBot Bot { get; }

        public override void Use(Player p, string message, CommandData data) {
            if (message.Length == 0) { Help(p); return; }
            string[] parts = message.SplitSpaces();
            GlobalRelayBot bot   = Bot;
                ...
                case "rank":
                    if (arg.Length == 0) {
                        p.Message("{0} controllers have the rank {1}", bot.RelayName,
                                  Group.GetColoredName(Server.Config.GlobalIRCControllerRank));
                        return;
                    }
                    
                    Group grp = Matcher.FindRanks(p, arg);
                    if (grp == null) return;
                    if (Server.Config.GlobalIRCControllerRank > data.Rank) {
                    ...
                    Server.Config.GlobalIRCControllerRank = grp.Permission;
                    SrvProperties.Save();
