...
namespace GoldenSparks.Modules.Relay1.Discord1
{
    ...
    public sealed class DiscordApiClient1 : RelayBotSender1<DiscordApiMessage1>
    {
        ...
        public override string ThreadName { get { return "Discord1-ApiClient"; } }
        ...
        static void LogError(Exception ex, DiscordApiMessage1 msg) {
            ...
            Logger.LogError("Error sending request to Discord1 API " + target, ex);
        }
        
        static void LogWarning(Exception ex) {
            Logger.Log(LogType.Warning, "Error sending request to Discord1 API - " + ex.Message);
        }
        
        static void LogResponse(string err) {
            if (string.IsNullOrEmpty(err)) return;
            Logger.Log(LogType.Warning, "Discord1 API returned: " + err);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res) {
            ...
            Logger.Log(LogType.SystemActivity, "Discord1 bot ratelimited! Trying again in {0} seconds..", delay);
