...
namespace GoldenSparks.Modules.Relay2.Discord2
{
    /// <summary> Represents an abstract Discord API message </summary>
    public abstract class DiscordApiMessage2
    {
        ...
        public virtual bool CombineWith(DiscordApiMessage2 prior) { return false; }
    }
    
    /// <summary> Message for sending text to a channel </summary>
    public class ChannelSendMessage : DiscordApiMessage2
    {
        ...
        public override bool CombineWith(DiscordApiMessage2 prior) {
    ...
    public class ChannelSendEmbed : DiscordApiMessage2
    {
        public string Title;
    ...
    public sealed class DiscordApiClient2 : RelayBotSender2<DiscordApiMessage2>
    {
        public string Token;
        const string host = "https://discord.com/api/v8";
        
        DiscordApiMessage2 GetNextRequest() {
            if (requests.Count == 0) return null;
            DiscordApiMessage2 first = requests.Dequeue();
            
            // try to combine messages to minimise API calls
            while (requests.Count > 0) {
                DiscordApiMessage2 next = requests.Peek();
        ...
        public override string ThreadName { get { return "Discord2-ApiClient"; } }
        public override void HandleNext() {
            DiscordApiMessage2 msg1 = null;
        ...
        static void LogError(Exception ex, DiscordApiMessage2 msg) {
            string target = "(" + msg.Method + " " + msg.Path + ")";
            Logger.LogError("Error sending request to Discord2 API " + target, ex);
        }
        
        static void LogWarning(Exception ex) {
            Logger.Log(LogType.Warning, "Error sending request to Discord2 API - " + ex.Message);
        }
        
        static void LogResponse(string err) {
            if (string.IsNullOrEmpty(err)) return;
            Logger.Log(LogType.Warning, "Discord2 API returned: " + err);
        }
        ...
        static void SleepForRetryPeriod(WebResponse res) {
            ...
            Logger.Log(LogType.SystemActivity, "Discord2 bot ratelimited! Trying again in {0} seconds..", delay);
