...
using GoldenSparks.Modules.Relay2;
using GoldenSparks.Network;
using Sharkbite2.Irc2;

namespace GoldenSparks.Modules.Relay2.IRC2 
{    
    /// <summary> Manages a list of IRC2 nicks and asssociated permissions </summary>
    sealed class IRCNickList2 
    {
        ...
        public bool VerifyNick(string channel, string userNick, ref string error, ref bool foundAtAll) {
            ...
            IRCControllerVerify2 verify = Server.Config.IRCVerify2;
            if (verify == IRCControllerVerify2.None) return true;
            
            if (verify == IRCControllerVerify2.HalfOp) {
                string prefix = GetPrefix(chanNicks[index]);
                if (prefix.Length == 0 || prefix == "+") {
                    error = "You must be at least a half-op on the channel to use commands from IRC2."; return false;
                }
                return true;
            } else {
                ...
                error = "You must have joined the opchannel to use commands from IRC2."; return false;
            }
        }
    }
}
