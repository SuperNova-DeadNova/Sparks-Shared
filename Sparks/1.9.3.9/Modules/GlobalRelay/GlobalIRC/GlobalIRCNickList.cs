...
using GoldenSparks.Modules.GlobalRelay;
using GoldenSparks.Network;
using Sharkbite2.Irc2;

namespace GoldenSparks.Modules.GlobalRelay.GlobalIRC 
{    
    /// <summary> Manages a list of GlobalIRC nicks and asssociated permissions </summary>
    sealed class GlobalIRCNickList 
    {
        Dictionary<string, List<string>> userMap = new Dictionary<string, List<string>>();     
        public GlobalIRCBot bot;
        ...
        public bool VerifyNick(string channel, string userNick, ref string error, ref bool foundAtAll) {
            ...
            GlobalIRCControllerVerify verify = Server.Config.GlobalIRCVerify;
            if (verify == GlobalIRCControllerVerify.None) return true;
            
            if (verify == GlobalIRCControllerVerify.HalfOp) {
                string prefix = GetPrefix(chanNicks[index]);
                if (prefix.Length == 0 || prefix == "+") {
                    error = "You must be at least a half-op on the channel to use commands from GlobalIRC."; return false;
                }
                return true;
            } else {
                ...
                error = "You must have joined the opchannel to use commands from GlobalIRC."; return false;
            }
        }
    }
}
