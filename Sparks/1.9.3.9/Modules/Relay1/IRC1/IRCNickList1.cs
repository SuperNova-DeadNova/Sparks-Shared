...
{    
    ...
    sealed class IRCNickList1 
    {
        ...
        public bool VerifyNick(string channel, string userNick, ref string error, ref bool foundAtAll) {
            ...
            IRCControllerVerify1 verify = Server.Config.IRCVerify1;
            if (verify == IRCControllerVerify1.None) return true;
            
            if (verify == IRCControllerVerify1.HalfOp) {
                string prefix = GetPrefix(chanNicks[index]);
                if (prefix.Length == 0 || prefix == "+") {
                    error = "You must be at least a half-op on the channel to use commands from IRC1."; return false;
                }
                return true;
            } else {
                ...
                error = "You must have joined the opchannel to use commands from IRC1."; return false;
            }
        }
    }
}
