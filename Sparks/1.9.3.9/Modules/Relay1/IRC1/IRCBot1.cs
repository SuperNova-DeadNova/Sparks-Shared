...
using GoldenSparks.Modules.Relay1;
using Sharkbite2.Irc2;

namespace GoldenSparks.Modules.Relay1.IRC1 
{
    public enum IRCControllerVerify1 { None, HalfOp, OpChannel };
    
    /// <summary> Manages a connection to an IRC server, and handles associated events. </summary>
    public sealed class IRCBot1 : RelayBot1 
    {
        ...
        public override string RelayName { get { return "IRC1"; } }
        public override bool Enabled  { get { return Server.Config.UseIRC1; } }
        public override string UserID { get { return conn == null ? null : conn.Nick; } }
        
        public override void LoadControllers() {
            Controllers = PlayerList.Load("ranks/IRC_Controllers1.txt");
        }
        
        public IRCBot1() {
            nicks     = new IRCNickList1();
            nicks.bot = this;
        }
        ...
        public override void DoConnect() {
            ready = false;
            nick  = Server.Config.IRCNick1.Replace(" ", "");
            
            if (conn == null) conn = new Connection(new UTF8Encoding(false));
            conn.Hostname = Server.Config.IRCServer1;
            conn.Port     = Server.Config.IRCPort1;
            conn.UseSSL   = Server.Config.IRCSSL1;
            ...
            bool usePass = Server.Config.IRCIdentify1 && Server.Config.IRCPassword1.Length > 0;
            conn.ServerPassword = usePass ? Server.Config.IRCPassword1 : "*";
            conn.Connect();
        }
        ...
        public override void UpdateConfig() {
            Channels     = Server.Config.IRCChannels1.SplitComma();
            OpChannels   = Server.Config.IRCOpChannels1.SplitComma();
            IgnoredUsers = Server.Config.IRCIgnored1.SplitComma();
            LoadBannedCommands();
        }
        ...
        void OnAction(UserInfo user, string channel, string description) {
            MessageInGame(user.Nick, string.Format("&I(IRC1) * {0} {1}", user.Nick, description));
        }
        ...
        void AnnounceJoinLeave(string nick, string verb, string channel) {
            ...
            MessageInGame(nick, string.Format("&I(IRC1) {0} {1} the{2} channel", nick, verb, which));
        }
        ...
        void OnQuit(UserInfo user, string reason) {
            ...
            if (user.Nick == nick) return;
            Logger.Log(LogType.RelayActivity, user.Nick + " left IRC1");
            MessageInGame(user.Nick, "&I(IRC1) " + user.Nick + " left");
        }

        void OnError(ReplyCode code, string message) {
            Logger.Log(LogType.RelayActivity, "IRC1 Error: " + message);
        }

        void OnPrivate(UserInfo user, string message) {
            RelayUser1 rUser = new RelayUser1();
            rUser.ID        = user.Nick;
        ...
        void JoinChannels() {
            Logger.Log(LogType.RelayActivity, "Joining IRC1 channels...");
        ...
        void Authenticate() {
            string nickServ = Server.Config.IRCNickServName1;
            if (nickServ.Length == 0) return;
            
            if (Server.Config.IRCIdentify1 && Server.Config.IRCPassword1.Length > 0) {
                Logger.Log(LogType.RelayActivity, "Identifying with " + nickServ);
                conn.SendMessage(nickServ, "IDENTIFY " + Server.Config.IRCPassword1);
            }
        }
        ...
        void OnNick(UserInfo user, string newNick) {
            ...
            MessageInGame(user.Nick, "&I(IRC1) " + user.Nick + " &Sis now known as &I" + newNick);
        }
        ...
        void OnKick(UserInfo user, string channel, string kickee, string reason) {
            ...
            Logger.Log(LogType.RelayActivity, "{0} kicked {1} from IRC1{2}", user.Nick, kickee, reason);
            MessageInGame(user.Nick, "&I(IRC1) " + user.Nick + " kicked " + kickee + reason);
        }
