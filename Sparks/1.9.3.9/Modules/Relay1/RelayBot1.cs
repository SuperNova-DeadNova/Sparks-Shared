...
namespace GoldenSparks.Modules.Relay1 
{
    public class RelayUser1 { public string ID, Nick; }

    public delegate void OnDirectMessage(RelayBot1 bot, string channel, RelayUser1 user, string message, ref bool cancel);
    ...
    {
        public static void Call(RelayBot1 bot, string channel, RelayUser1 user, string message, ref bool cancel) {
    ...
    public delegate void OnChannelMessage(RelayBot1 bot, string channel, RelayUser1 user, string message, ref bool cancel);
    ...
    public abstract class RelayBot1 
    {
        ...
        readonly Player fakeGuest = new Player("RelayBot1");
        readonly Player fakeStaff = new Player("RelayBot1");
        ...
        public string Connect() {
            ...
            if (Server.shuttingDown) return "cannot connect as server is shutting down";
            canReconnect = true;
            retries      = 0;
        ...
        public void LoadBannedCommands() {
            BannedCommands = new List<string>() { "IRCBot1", "DiscordBot1", "OpRules", "IRCControllers1", "DiscordControllers1" };
            
            if (!File.Exists("text/irccmdblacklist1.txt")) {
                File.WriteAllLines("text/irccmdblacklist1.txt", new string[] {
                                       "# Here you can put commands that cannot be used from the IRC bot.",
                                       "# Lines starting with \"#\" are ignored." });
            }
            
            foreach (string line in File.ReadAllLines("text/irccmdblacklist1.txt")) {
        ...
        static bool FilterIRC(Player pl, object arg) {
            return !pl.Ignores.IRC1 && !pl.Ignores.IRCNicks1.Contains((string)arg);
        ...
        public void HandleDirectMessage(RelayUser1 user, string channel, string message) {
        ...
        public void HandleChannelMessage(RelayUser1 user, string channel, string message) {
            if (IgnoredUsers.CaselessContains(user.ID)) return;
            message = ParseMessage(message).TrimEnd();
            if (message.Length == 0) return;
        ...
        bool HandleListPlayers(RelayUser1 user, string channel, string cmd, bool opchat) {
            ...
            try {
                RelayPlayer1 p = new RelayPlayer1(channel, user, this);
        ...
        public virtual void MessagePlayers(RelayPlayer1 p) {
            Command.Find("Players").Use(p, "", p.DefaultCmdData);
        }
        
                
        bool HandleCommand(RelayUser1 user, string channel, string message, string[] parts) {
        ...
        bool ExecuteCommand(RelayUser1 user, string channel, string cmdName, string cmdArgs) {
            Command cmd = Command.Find(cmdName);
            Player p = new RelayPlayer1(channel, user, this);
        ...
        public bool CanUseCommand(RelayUser1 user, string cmd, out string error) {
            error = null;
        ...
        public sealed class RelayPlayer1 : Player {
            public readonly string ChannelID;
            public readonly RelayUser1 User;
            public readonly RelayBot1 Bot;
            
            public RelayPlayer1(string channel, RelayUser1 user, RelayBot1 bot) : base(bot.RelayName) {
                group = Group.Find(Server.Config.IRCControllerRank1);
