...
namespace GoldenSparks.Games {
    ...
    public sealed partial class ZSGame : RoundsGame {
        ...
        void CheckHumanPledge(Player p, Player killer) {
            ...
            if (killer == null) {
                p.Message("As this was an automatic infection, you have not lost any &3" + Server.Config.Currency + " :D");
            } else {
                p.SetMoney(Math.Max(p.money - 5, 0));
            }
        }
        ...
        void GiveMoney(Player[] alive) {
            ...
            foreach (Player pl in online) {
                ...
                if (pl.Game.Referee) {
                    pl.Message("You gained one " + Server.Config.Currency + " because you're a ref. Would you like a medal as well? :P");
