...
namespace GoldenSparks.UI
{
    ...
    public static class UIHelpers
    {
        ...
        public static void RepeatCommand()
        {
            if (lastCMD.Length == 0)
            {
                Logger.Log(LogType.CommandUsage, "(Sparks): Cannot repeat command - no commands used yet.");
                return;
            }
        ...
        public static void HandleCommand(string text)
        {
            ...
            if (cmd == null)
            {
                Logger.Log(LogType.CommandUsage, "(Sparks): Unknown command \"{0}\"", name); return;
            }
            if (!cmd.SuperUseable)
            {
                Logger.Log(LogType.CommandUsage, "(Sparks): /{0} can only be used in-game.", cmd.name); return;
            }
            ...
            Thread thread = new Thread(
                () => {
                    try
                    {
                        cmd.Use(Player.Sparks, args);
                        if (args.Length == 0)
                        {
                            Logger.Log(LogType.CommandUsage, "(Sparks) used /" + cmd.name);
                        }
                        else
                        {
                            Logger.Log(LogType.CommandUsage, "(Sparks) used /" + cmd.name + " " + args);
                        }
                    }
                    catch (Exception ex)
                    {
