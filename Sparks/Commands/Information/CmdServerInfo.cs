...
namespace GoldenSparks.Commands.Info {
    public sealed class CmdServerInfo : Command2 {
        ...
        public override void Use(Player p, string message, CommandData data) {
            ...
            p.Message("Been up for &b{0}&S, running &b{1} &f" + Updater.SourceURL,
                      up.Shorten(true), Server.SoftwareNameVersioned);
            ...
            if (!owner.CaselessEq("Notch") && !owner.CaselessEq("GoldenSparks"))
            {
                p.Message("Owner is &3{0}. &SCore state: &3{1}", owner, Server.Config.CoreState);
            }
            else
            {
                p.Message("Core state: &3{0}", Server.Config.CoreState);
            }
